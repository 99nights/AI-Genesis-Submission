...
// Internal helper: Fetch products directly by productIds (avoids fetching items again)
// Exported for use by dataLoader to avoid duplicate item fetching
export const getCanonicalProductsForShopDirect = async (productIds: Set<string>): Promise<ProductDefinition[]> => {
  if (!qdrantClient || productIds.size === 0) return [];
  ...
  do {
    ...
    if (!offset || points.length > 10_000) break;
  } while (offset);
...
export const getCanonicalProductsForShop = async (shopId?: string | null): Promise<ProductDefinition[]> => {
  ...
  // Get ALL items for this shop (including empty/expired items) using fetchAllPoints directly
  ...
  // Extract unique productIds from items (regardless of status or quantity)
  ...
  // Use optimized direct fetch to avoid redundant item fetching
  return getCanonicalProductsForShopDirect(productIds);
};
